...
void WbInertialUnit::writeAnswer(QDataStream &stream) {
  if (refreshSensorIfNeeded() || mSensor->hasPendingValue()) {
    ...
    stream << (double)mMatrixValues(0, 0) << (double)mMatrixValues(0, 1) << (double)mMatrixValues(0, 2);
    stream << (double)mMatrixValues(1, 0) << (double)mMatrixValues(1, 1) << (double)mMatrixValues(1, 2);
    stream << (double)mMatrixValues(2, 0) << (double)mMatrixValues(2, 1) << (double)mMatrixValues(2, 2);
...
void WbInertialUnit::computeValue() {
  ...
  rm.transpose();
  mMatrixValues = rotationMatrix() * rm;  // extrensic rotation matrix e = Y(yaw) Z(pitch) X(roll) w.r.t reference frame
  const double roll = atan2(-mMatrixValues(1, 2), mMatrixValues(1, 1));
  const double pitch = asin(mMatrixValues(1, 0));
  assert(!std::isnan(pitch));
  const double yaw = -atan2(mMatrixValues(2, 0), mMatrixValues(0, 0));
